# SEO And Discovery

# SEO & DISCOVERY

Project

OneBite Bakery Platform

Brand

OneBite Bakery

Tagline

हर जश्न का पहला निवाला।

Version

1.0

Purpose

Make the OneBite Bakery Platform easy to discover through search engines, social media, maps, and AI-powered search.

SEO should be built into the platform from day one.

---

# GOALS

- Rank on Google
- Improve local discovery
- Increase organic traffic
- Improve product visibility
- Improve social sharing
- Support AI search engines

---

# URL STRUCTURE

Home

/

Products

/products

Categories

/categories

Product

/products/:slug

Example

/products/chocolate-truffle-cake

Never expose IDs in public URLs.

---

# PRODUCT SLUGS

Generate readable slugs.

Example

Chocolate Truffle Cake

↓

chocolate-truffle-cake

Slug must be unique.

---

# META TAGS

Every page must contain

Title

Description

Keywords

Canonical URL

Robots

Author

Theme Color

---

# PRODUCT META

Each product should have

SEO Title

SEO Description

SEO Keywords

Image Alt Text

Open Graph Image

---

# OPEN GRAPH

Support

WhatsApp

Facebook

LinkedIn

Discord

Preview should include

Product Image

Title

Price

Short Description

---

# TWITTER CARDS

Enable Twitter/X preview cards.

---

# STRUCTURED DATA

Use Schema.org

Website

Organization

Local Business

Product

Breadcrumb

Review

FAQ

---

# LOCAL SEO

Business Name

Phone

Address

Opening Hours

Google Maps Location

Google Business Profile

---

# SITEMAP

Generate automatically.

Include

Products

Categories

Pages

Exclude

Admin

Private APIs

---

# ROBOTS.TXT

Allow

Public Pages

Disallow

Admin

Private APIs

Temporary files

---

# IMAGE SEO

Every image should include

Alt Text

Responsive Sizes

Lazy Loading

WebP

Compression

Meaningful filenames

Example

black-forest-cake.webp

---

# PERFORMANCE

Target

Lighthouse 90+

Largest Contentful Paint < 2.5s

CLS < 0.1

INP within Google's recommendation

---

# BREADCRUMBS

Example

Home

>

Cakes

>

Chocolate Cakes

>

Chocolate Truffle Cake

---

# SEARCH ENGINE INDEXING

Index

Products

Categories

Blog (Future)

FAQ

Policies

Do Not Index

Admin

Checkout

Login

Profile

---

# SOCIAL SHARING

Every product should support

WhatsApp

Facebook

Instagram Copy Link

Copy URL

---

# REVIEWS

Expose ratings using structured data.

Improve Google search appearance.

---

# FAQ

Support FAQ structured data.

Examples

Delivery Time

Pickup Process

Custom Cake

Payments

---

# AI DISCOVERY

Write clear product descriptions.

Use semantic headings.

Avoid keyword stuffing.

Maintain accurate business information.

---

# FUTURE

Google Merchant Center

Google Shopping

Recipe Schema

Blog

Seasonal Landing Pages

Voice Search Optimization

---

# SUCCESS METRICS

Organic Visitors

Top Search Queries

Indexed Pages

Page Speed

CTR

Average Position

Conversion Rate

---

# GOLDEN RULE

Every public page should be optimized for humans first and search engines second.
